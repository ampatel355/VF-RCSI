...
from Code.asset_class_universe import ASSET_CLASS_DISPLAY_NAMES, SUPPORTED_ASSET_CLASSES
...
ASSET_CLASS_OVERRIDE_OPTIONS = ["auto"] + [
    asset_class
    for asset_class in SUPPORTED_ASSET_CLASSES
    if asset_class != "manual_override"
]
...
def strategy_label(agent_name: str, *, short: bool = True) -> str:
    """Return a readable strategy label for compact UI elements."""
...
def render_single_workflow_summary(ticker: str, result: WorkflowRunResult | None) -> None:
    ...
    if best_row is not None:
        summary_cols[0].metric(
            ...
            strategy_label(str(best_row["agent"]), short=True),
    ...
    if p_value_row is not None:
        summary_cols[1].metric(
            "Lowest p-value",
            strategy_label(str(p_value_row["agent"]), short=True),
    ...
    if robustness_df is not None and "agent" in robustness_df.columns:
        ...
        if not robust_row.empty:
            ...
            summary_cols[2].metric(
                ...
                strategy_label(str(best_robust["agent"]), short=True),
...
def render_single_metrics_tab(ticker: str) -> None:
    ...
    relative_strength_universe_path = data_clean_dir() / f"{ticker}_momentum_relative_strength_universe.csv"
    for label, path in [
        ("Raw Price History", raw_path),
        ("Feature Table", features_path),
        ("Regime Table", regimes_path),
        ("Relative Strength Universe", relative_strength_universe_path),
    ]:
...
def render_single_trades_tab(ticker: str) -> None:
    ...
    for path in trade_paths:
        ...
        title = strategy_label(agent_name, short=False)
...
def render_walk_forward_summary(result: WorkflowRunResult | None) -> None:
    ...
    if "mean_p_value" in agent_df.columns:
        ...
        if not working_df.empty:
            ...
            st.metric(
                "Lowest mean p-value",
                strategy_label(str(top_row["agent"]), short=True),
...
def render_shared_parameter_controls(prefix: str) -> dict[str, str]:
    ...
    with st.expander("Strategy Parameters", expanded=False):
        relative_strength_lookback = st.number_input(
            "Relative strength lookback days",
            min_value=1,
            value=60,
            key=f"{prefix}_relative_strength_lookback",
        )
        ...
        vol_managed_entry_lookback = st.number_input(
            "Vol-managed TSMOM entry lookback days",
            min_value=2,
            value=int(VOL_MANAGED_TSMOM_ENTRY_LOOKBACK_DAYS),
            key=f"{prefix}_vol_managed_entry_lookback",
        )
        vol_managed_exit_lookback = st.number_input(
            "Vol-managed TSMOM exit lookback days",
            min_value=1,
            value=int(VOL_MANAGED_TSMOM_EXIT_LOOKBACK_DAYS),
            key=f"{prefix}_vol_managed_exit_lookback",
        )
        vol_managed_vol_lookback = st.number_input(
            "Vol-managed TSMOM volatility lookback days",
            min_value=2,
            value=int(VOL_MANAGED_TSMOM_VOL_LOOKBACK_DAYS),
            key=f"{prefix}_vol_managed_vol_lookback",
        )
        vol_managed_target_vol = st.number_input(
            "Vol-managed TSMOM target annualized vol",
            min_value=0.01,
            max_value=1.0,
            value=float(VOL_MANAGED_TSMOM_TARGET_ANNUALIZED_VOL),
            step=0.01,
            format="%.2f",
            key=f"{prefix}_vol_managed_target_vol",
        )
        vol_managed_min_fraction = st.number_input(
            "Vol-managed TSMOM min capital fraction",
            min_value=0.01,
            max_value=1.0,
            value=float(VOL_MANAGED_TSMOM_MIN_CAPITAL_FRACTION),
            step=0.01,
            format="%.2f",
            key=f"{prefix}_vol_managed_min_fraction",
        )
        vol_managed_max_fraction = st.number_input(
            "Vol-managed TSMOM max capital fraction",
            min_value=0.01,
            max_value=1.0,
            value=float(VOL_MANAGED_TSMOM_MAX_CAPITAL_FRACTION),
            step=0.01,
            format="%.2f",
            key=f"{prefix}_vol_managed_max_fraction",
        )
        vol_managed_stop_atr = st.number_input(
            "Vol-managed TSMOM stop ATR multiplier",
            min_value=0.1,
            value=float(VOL_MANAGED_TSMOM_STOP_ATR_MULTIPLIER),
            step=0.1,
            format="%.1f",
            key=f"{prefix}_vol_managed_stop_atr",
        )
        relative_strength_universe = st.text_input(
            "Relative strength manual universe override",
            value="",
            key=f"{prefix}_relative_strength_universe",
            help=(
                "Optional. Leave blank to auto-select a same-asset-class peer universe from the active ticker. "
                "Populate this only when you intentionally want a custom peer list."
            ),
        )
        relative_strength_asset_class = st.selectbox(
            "Relative strength asset class override",
            options=ASSET_CLASS_OVERRIDE_OPTIONS,
            index=0,
            format_func=lambda asset_class: (
                "Auto-detect from ticker"
                if asset_class == "auto"
                else ASSET_CLASS_DISPLAY_NAMES.get(asset_class, asset_class.replace("_", " ").title())
            ),
            key=f"{prefix}_relative_strength_asset_class",
            help=(
                "Optional. Auto-detect keeps ranking inside the selected ticker's peer asset class. "
                "Use an override only if you want to force a specific peer bucket."
            ),
        )
        relative_strength_rebalance = st.selectbox(
            "Relative strength rebalance",
            options=["weekly", "monthly"],
            index=1,
            key=f"{prefix}_relative_strength_rebalance",
        )
    ...
    env_overrides.update(
        {
            "RELATIVE_STRENGTH_LOOKBACK_DAYS": str(int(relative_strength_lookback)),
            "MOMENTUM_LOOKBACK_DAYS": str(int(relative_strength_lookback)),
            "BREAKOUT_LOOKBACK_DAYS": str(int(breakout_lookback)),
            "VOL_MANAGED_TSMOM_ENTRY_LOOKBACK_DAYS": str(int(vol_managed_entry_lookback)),
            "VOL_MANAGED_TSMOM_EXIT_LOOKBACK_DAYS": str(int(vol_managed_exit_lookback)),
            "VOL_MANAGED_TSMOM_VOL_LOOKBACK_DAYS": str(int(vol_managed_vol_lookback)),
            "VOL_MANAGED_TSMOM_TARGET_ANNUALIZED_VOL": str(float(vol_managed_target_vol)),
            "VOL_MANAGED_TSMOM_MIN_CAPITAL_FRACTION": str(float(vol_managed_min_fraction)),
            "VOL_MANAGED_TSMOM_MAX_CAPITAL_FRACTION": str(float(vol_managed_max_fraction)),
            "VOL_MANAGED_TSMOM_STOP_ATR_MULTIPLIER": str(float(vol_managed_stop_atr)),
            "RELATIVE_STRENGTH_UNIVERSE": relative_strength_universe,
            "RELATIVE_STRENGTH_FORCE_ASSET_CLASS": (
                "" if relative_strength_asset_class == "auto" else relative_strength_asset_class
            ),
            "RELATIVE_STRENGTH_REBALANCE_FREQUENCY": relative_strength_rebalance,
...
def main() -> None:
    ...
    st.sidebar.multiselect(
        ...
        format_func=lambda name: strategy_label(name, short=True),
        disabled=True,
    ...
    st.sidebar.info(
        ...
        "The interface preserves that behavior while the relative-strength strategy now auto-selects a same-asset-class peer universe unless you explicitly override it."
    )
