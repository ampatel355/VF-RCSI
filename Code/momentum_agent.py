"""Simulate a time-series momentum trading agent with next-bar execution."""
...
try:
    ...
    from strategy_config import MOMENTUM_LOOKBACK_DAYS, MOMENTUM_RETURN_COLUMN
    from strategy_simulator import resolve_data_clean_dir, run_strategy
except ModuleNotFoundError:
    from Code.regimes import main as create_regimes
    from Code.strategy_config import MOMENTUM_LOOKBACK_DAYS, MOMENTUM_RETURN_COLUMN
...
def load_regime_data(input_path: Path) -> pd.DataFrame:
    ...
    df[MOMENTUM_RETURN_COLUMN] = pd.to_numeric(df[MOMENTUM_RETURN_COLUMN], errors="coerce")
...
def main() -> None:
    """Run the momentum strategy and save its completed trades."""
    project_root = Path(__file__).resolve().parents[1]
    data_clean_dir = resolve_data_clean_dir(project_root)
    input_path = data_clean_dir / f"{ticker}_regimes.csv"
    output_path = data_clean_dir / f"{ticker}_momentum_trades.csv"

    df = load_regime_data(input_path)
    trades_df = run_strategy("momentum", df)
    trades_df.to_csv(output_path, index=False)

    print(f"Momentum lookback days: {MOMENTUM_LOOKBACK_DAYS}")
