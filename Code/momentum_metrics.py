"""Calculate simple performance metrics for the momentum agent."""
...
try:
    ...
    from momentum_agent import main as create_trades
except ModuleNotFoundError:
    from Code.research_metrics import calculate_trade_level_return_ratio
    from Code.momentum_agent import main as create_trades
...
def main() -> None:
    """Calculate and save the momentum strategy metrics."""
    project_root = Path(__file__).resolve().parents[1]
    data_clean_dir = resolve_data_clean_dir(project_root)
    input_path = data_clean_dir / f"{ticker}_momentum_trades.csv"
    output_path = data_clean_dir / f"{ticker}_momentum_metrics.csv"
